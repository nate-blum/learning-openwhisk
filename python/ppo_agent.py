...
class PPO:
    ...
    def __init__(self, env: Cluster, eval_env, func_state_dim, num_func, cluster_state_dim,
                 action_dim,
        ...
        self.server_type_lst = self.env.get_attr('server_type_lst')[0]
        self.n_epochs = epoch
        self.batch_size = batch_size
        self.buffers = RolloutBuffer(size=trajectory_len, n_env=self.num_evn, obs_dim=self.obs_dim, action_dim=action_dim,
    ...
    def collect_rollouts(self, T: int):
        ...
        # pprint(states)
        for t in range(T):
            actions, action_logprobs = self.select_action(states, self.stds)  # return is of Tensor type
            self.buffers.states[t] = np.array(states).copy()
            actions_np = actions.detach().cpu().numpy()
            # [rewards_dict, rewards_dict2]
            states, rewards, terminated, truncated, infos = self.env.step(actions_np)  # NOTE, do I need detach
            self.buffers.actions[t] = np.array(actions_np).copy()
            # print(self.buffers.logprobs[t].shape,action_logprobs.clone().cpu().numpy().shape )
            self.buffers.logprobs[t] = action_logprobs.clone().cpu().numpy()
            rewards_all = [r['all'] for r in rewards]
            self.buffers.rewards[t] = np.array(rewards_all).copy()
    ...
    def _compute_advantage(self) -> np.ndarray:
        ...
        gains = np.zeros((T, self.num_env), dtype=np.float32)  # shape [T, n_envs]
        discounted_reward = np.zeros(self.num_env)
        ...
        baselines_mean = np.expand_dims(baselines_mean, 1)  # [T, 1]
        baselines_std = np.expand_dims(baselines_std, 1)
        baselines_mean = np.repeat(baselines_mean, self.num_env, axis=1)  # [T, n_env]
        baselines_std = np.repeat(baselines_std, self.num_env, axis=1)
    ...
    def train(self):
        ...
        while num_update < self.max_n_update:
            t = time.time()
            self.collect_rollouts(self.trajectory_len)
    ...
    def evaluate(self, step):
        ...
        for t in range(T):
            ...
            mapped_action = self.eval_env.map_action_for_monitor(action_np)
            print(mapped_action)
            ...
            print(self.eval_env.get_core_status_str())
